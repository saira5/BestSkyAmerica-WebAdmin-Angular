...
export class StoreService {
  ...
  getStores() {
    return this.httpServices.post({'model':"StoresDetail"},'/api/common/get-data').pipe(
      catchError(err => of({ error: true, message: 'Server error', data: [] })));
  }

  getStoresByID(id) {
    return this.httpServices.post({"condition":id,'model':"StoresDetail"},'/api/common/getstorelistByID').pipe(
      catchError(err => of({ error: true, message: 'Server error', data: [] })));
  }
  ...
  getStoreById(id) {
    return this.httpServices.post({"_id":id,"model":"StoresDetail"},'/api/common/single').pipe(
      catchError(err => of({ error: true, message: 'Server error', data: [] })));
  }

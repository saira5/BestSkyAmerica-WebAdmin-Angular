...
export class BrandListingComponent implements OnInit {
  ...
  constructor(private BrandService: BrandService, private router: Router, private currentActivatedRoute: ActivatedRoute) { }
  ...
  getStaffList() {
    const staff = this.BrandService.getStores();
    staff.subscribe(
      result => {
  ...
  async deleteStoreById(data) {
    ...
    if (response == true) {
      this.loaded = false;
      this.BrandService.deleteById(data).subscribe(
        result => {
          if (!result['error']) {
  ...
  async deleteMultipleStaff() {
    if (this.staff_ids.length > 0) {
      ...
      if (response == true) {
        this.loaded = false;
        this.BrandService.deleteMultipeStaff(this.staff_ids).subscribe(
          result => {
            if (!result['error']) {

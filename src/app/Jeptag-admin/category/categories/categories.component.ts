...
export class CategoriesComponent implements OnInit {
  ...
  constructor(private CategoryService: CategoryService, private router: Router, private currentActivatedRoute: ActivatedRoute) { }
  ...
  getStaffList() {
    const staff = this.CategoryService.getStores();
    staff.subscribe(
      result => {
  ...
  async deleteStoreById(data) {
    ...
    if (response == true) {
      this.loaded = false;
      this.CategoryService.deleteById(data).subscribe(
        result => {
          if (!result['error']) {
            SwalAlert.sucessAlert('', 'Category Deleted Successfully!');
            this.getStaffList();
          }
          else {
            this.loaded = true;
  ...
  async deleteMultipleStaff() {
    if (this.staff_ids.length > 0) {
      ...
      if (response == true) {
        this.loaded = false;
        this.CategoryService.deleteMultipeStaff(this.staff_ids).subscribe(
          result => {
            if (!result['error']) {
